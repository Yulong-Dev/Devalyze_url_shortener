...
const LinksPage = () => {
  ...
  const [maxLength, setMaxLength] = useState(window.innerWidth < 640 ? 30 : 60);

  useEffect(() => {
    loadLinks();

    // Update truncation length on screen resize
    const handleResize = () => {
      setMaxLength(window.innerWidth < 640 ? 30 : 60);
    };
  ...
  return (
    <div className="bg-gray-100 flex flex-col min-h-screen max-w-screen gap-6 p-4 sm:p-6">
              ...
              return (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p
                      className="font-medium truncate max-w-full block text-ellipsis overflow-hidden"
                      title={u.title || u.longUrl}
                    >
                      {truncateText(u.title || u.longUrl)}
                    </p>
                    <div className="flex flex-wrap items-center gap-3 mt-1 text-sm min-w-0">
                      {/* Long URL (truncated dynamically) */}
                      <a
                        href={u.longUrl}
                        target="_blank"
                        ...
                        title={u.longUrl}
                      >
                        {truncateText(u.longUrl)}
                      </a>
                      <span className="hidden sm:inline">→</span>
                      {/* Short URL (truncated dynamically) */}
                      <a
                        href={short}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-blue-600 hover:underline flex items-center gap-1 block"
                        title={short}
                      >
                        {truncateText(short)}{" "}
